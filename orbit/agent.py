...
ORBIT_INSTRUCTION = (
    ...
    "these work in any task and never touch the mouse/keyboard. "
    "perception_get_state is effectively free; call it first when you need "
    "to know what's currently on screen. perception_find_element needs "
    "automation_id or name (from perception_get_uia_tree, or from "
    "context) — it is NOT free-text visual search; there is no vision-tier "
    "fallback in this build, so an element with no UI Automation "
    "representation (some custom-drawn UI) cannot be resolved at all. "
    "perception_capture_screenshot returns a base64 PNG for the user to "
    "look at; it is not itself something you can visually interpret."
)
